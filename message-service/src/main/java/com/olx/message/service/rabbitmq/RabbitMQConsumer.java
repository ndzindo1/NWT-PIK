package com.olx.message.service.rabbitmq;
...
public class RabbitMQConsumer {
	...
	@RabbitListener(queues = "${queue.name.message}")
	...
	@RabbitListener(queues = "${queue.name.message.delete}")
	public void deleteUser(String email) {
		//Desila se greska u nekom servisu, brisemo usera ako ima
		if(userManager.getUserByEmail(email) != null) {
			userManager.delete(email);
		}
		System.out.println("Pokrenuta inverza akcija - message!");
	}
}
